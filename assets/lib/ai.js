...
(function (behaviourTree) {
    ...
    var Behavior = /** @class */ (function () {
         ...
         * @returns
         */
        Behavior.prototype.tick = function (context, dt) {
            if (this.status == behaviourTree.TaskStatus.Invalid)
                this.onStart();
            this.status = this.update(context, dt);
        ...
        BehaviorTree.prototype.tick = function (dt) {
            ...
            if (this.updatePeriod > 0) {
                ...
                if (this._elapsedTime <= 0) {
                    ...
                    this._root.tick(this._context, dt);
                }
            }
            else {
                this._root.tick(this._context, dt);
...
(function (behaviourTree) {
    ...
    var BehaviorTreeReference = /** @class */ (function (_super) {
        ...
        BehaviorTreeReference.prototype.update = function (context, dt) {
            this._childTree.tick(dt);
...
(function (behaviourTree) {
    ...
    var WaitAciton = /** @class */ (function (_super) {
        ...
        WaitAciton.prototype.update = function (context, dt) {
            // 我们不能使用Time.deltaTime，因为行为树会按照自己的速率tick，所以我们只存储起始时间
            this._startTime += dt;
            if (this._startTime >= this.waitTime)
                return behaviourTree.TaskStatus.Success;
...
(function (behaviourTree) {
    ...
    var Composite = /** @class */ (function (_super) {
         ...
         */
        Composite.prototype.updateSelfAbortConditional = function (context, statusCheck, dt) {
            // 检查任何IConditional的子任务，看它们是否改变了状态
            for (var i = 0; i < this._currentChildIndex; i++) {
                var child = this._children[i];
                if (!behaviourTree.isIConditional(child))
                    continue;
                var status_1 = this.updateConditionalNode(context, child, dt);
         ...
         */
        Composite.prototype.updateLowerPriorityAbortConditional = function (context, statusCheck, dt) {
            ...
            for (var i = 0; i < this._currentChildIndex; i++) {
                ...
                if (composite != null && behaviourTree.AbortTypesExt.has(composite.abortType, behaviourTree.AbortTypes.LowerPriority)) {
                    ...
                    var status_2 = this.updateConditionalNode(context, child, dt);
         ...
         * @returns
         */
        Composite.prototype.updateConditionalNode = function (context, node, dt) {
            if (node instanceof behaviourTree.ConditionalDecorator)
                return node.executeConditional(context, dt, true);
            else
                return node.update(context, dt);
        };
        return Composite;
    }(behaviourTree.Behavior));
...
(function (behaviourTree) {
    ...
    var Parallel = /** @class */ (function (_super) {
        ...
        Parallel.prototype.update = function (context, dt) {
...
(function (behaviourTree) {
    ...
    var ParallelSelector = /** @class */ (function (_super) {
        ...
        ParallelSelector.prototype.update = function (context, dt) {
            var didAllFail = true;
            for (var i = 0; i < this._children.length; i++) {
                var child = this._children[i];
                child.tick(context, dt);
...
(function (behaviourTree) {
    ...
    var Selector = /** @class */ (function (_super) {
        ...
        Selector.prototype.update = function (context, dt) {
            // 首先，如果我们不在第一个子节点身上，我们就处理条件性中止
            if (this._currentChildIndex != 0)
                this.handleConditionalAborts(context, dt);
            var current = this._children[this._currentChildIndex];
            var status = current.tick(context, dt);
            // 如果子节点成功了或者还在跑，就提前返回
        ...
        Selector.prototype.handleConditionalAborts = function (context, dt) {
            // 检查任何较低优先级的任务，看它们是否改变为成功
            if (this._hasLowerPriorityConditionalAbort)
                this.updateLowerPriorityAbortConditional(context, behaviourTree.TaskStatus.Failure, dt);
            if (behaviourTree.AbortTypesExt.has(this.abortType, behaviourTree.AbortTypes.Self))
                this.updateSelfAbortConditional(context, behaviourTree.TaskStatus.Failure, dt);
        };
        return Selector;
    }(behaviourTree.Composite));
...
(function (behaviourTree) {
    ...
    var Sequence = /** @class */ (function (_super) {
        ...
        Sequence.prototype.update = function (context, dt) {
        ...
        Sequence.prototype.handleConditionalAborts = function (context, dt) {
            if (this._hasLowerPriorityConditionalAbort)
                this.updateLowerPriorityAbortConditional(context, behaviourTree.TaskStatus.Success, dt);
            if (behaviourTree.AbortTypesExt.has(this.abortType, behaviourTree.AbortTypes.Self))
                this.updateSelfAbortConditional(context, behaviourTree.TaskStatus.Success, dt);
        };
        return Sequence;
    }(behaviourTree.Composite));
...
(function (behaviourTree) {
    ...
    var AlwaysFail = /** @class */ (function (_super) {
        ...
        AlwaysFail.prototype.update = function (context, dt) {
            behaviourTree.Assert.isNotNull(this.child, "child必须不能为空");
            var status = this.child.update(context, dt);
...
(function (behaviourTree) {
    ...
    var AlwaysSucceed = /** @class */ (function (_super) {
        ...
        AlwaysSucceed.prototype.update = function (context, dt) {
            behaviourTree.Assert.isNotNull(this.child, "child必须不能为空");
            var status = this.child.update(context, dt);
            if (status == behaviourTree.TaskStatus.Running)
                return behaviourTree.TaskStatus.Running;
...
(function (behaviourTree) {
    ...
    var ConditionalDecorator = /** @class */ (function (_super) {
        ...
        ConditionalDecorator.prototype.update = function (context, dt) {
            behaviourTree.Assert.isNotNull(this.child, "child不能为空");
            this._conditionalStatus = this.executeConditional(context, dt);
            if (this._conditionalStatus == behaviourTree.TaskStatus.Success)
                return this.child.tick(context, dt);
            return behaviourTree.TaskStatus.Failure;
        };
        /**
         ...
         */
        ConditionalDecorator.prototype.executeConditional = function (context, dt, forceUpdate) {
            if (forceUpdate === void 0) { forceUpdate = false; }
            if (forceUpdate || this._shouldReevaluate || this._conditionalStatus == behaviourTree.TaskStatus.Invalid)
                this._conditionalStatus = this._conditional.update(context, dt);
...
(function (behaviourTree) {
    ...
    var Inverter = /** @class */ (function (_super) {
        ...
        Inverter.prototype.update = function (context, dt) {
            behaviourTree.Assert.isNotNull(this.child, "child必须不能为空");
            var status = this.child.tick(context, dt);
            if (status == behaviourTree.TaskStatus.Success)
                return behaviourTree.TaskStatus.Failure;
            if (status == behaviourTree.TaskStatus.Failure)
                return behaviourTree.TaskStatus.Success;
...
(function (behaviourTree) {
    ...
    var Repeater = /** @class */ (function (_super) {
        ...
        Repeater.prototype.update = function (context, dt) {
            behaviourTree.Assert.isNotNull(this.child, "child必须不能为空");
            // 我们在这里和运行后检查，以防计数为0
...
(function (behaviourTree) {
    ...
    var UntilFail = /** @class */ (function (_super) {
        ...
        UntilFail.prototype.update = function (context, dt) {
...
(function (behaviourTree) {
    ...
    var UntilSuccess = /** @class */ (function (_super) {
        ...
        UntilSuccess.prototype.update = function (context, dt) {
            behaviourTree.Assert.isNotNull(this.child, "child必须不为空");
            var status = this.child.update(context, dt);
            if (status != behaviourTree.TaskStatus.Success)
                return behaviourTree.TaskStatus.Running;
